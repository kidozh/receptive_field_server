...
data = pd.read_excel(XLSX_FILE, sheet_name="thread_latency_EB")
...
for i in range(1, 5):
    ...
    plt.plot(N*0.064, data["latency%d" % (i)][N], label=LABEL_LIST[i] + " (%.3fs)" %(arr.mean()), linestyle=LINESTYLE_LIST[i])
