...
from client import ExponentialBackoffClient
...
expoentialBackoffClient = ExponentialBackoffClient(URL, SAMPLE_FREQ, SAMPLE_DURATION)
...
while True:
    ...
    while SAMPLE_END_TIME < subsampled_signal.shape[0]:
        ...
        signal_data = SignalData(timestamp, SAMPLE_FREQ, SAMPLE_DURATION, sample.tolist(),
                                 'Processing')
        # print("SEND", timestamp)
        isSent = expoentialBackoffClient.send_json_data_if_delay_permitted(signal_data)
        if isSent:
            print("SEND", timestamp)
